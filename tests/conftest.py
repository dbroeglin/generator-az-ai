...
class Solution:
    ...
    def run_in(self, cmd, path="", capture=False):
        if capture:
            result =  subprocess.run(f"cd {self.directory}/{path} && {cmd}", shell=True, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        else:
...
def solution(request, tmp_path_factory):
    ...
    if existing is None:
        ...
        solution.run_in(f"az-ai-generator-test '{dir}' {level}")

        yield solution

        shutil.rmtree(dir)
    else:
      yield Solution(existing)
...
def azd_env(request, solution):
    if os.getenv('SOLUTION') is None:
        ...
        solution.run_in((
            f'(test "$(az group exists --name {resource_group_name} -o tsv)" = "true" && '
